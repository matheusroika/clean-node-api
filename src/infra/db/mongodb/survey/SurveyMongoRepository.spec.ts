...
const makeFakeSurveyParamsWithDate = (): FakeSurveyParams => ({
  ...makeFakeSurveyParams(),
  date: new Date('2023-07-02T05:52:28.514Z')
})

type FakeSurveyParams = Omit<Survey, 'id'>

const makeFakeSurveys = (): FakeSurveyParams[] => ([
  makeFakeSurveyParamsWithDate(),
  {
...
describe('Survey MongoDB Repository', () => {
  ...
  describe('AddSurveyRepository', () => {
    test('Should add a survey on Survey.add success', async () => {
      ...
      await sut.add(makeFakeSurveyParams())
  ...
  describe('LoadSurveyByIdRepository', () => {
    test('Should load a survey on success', async () => {
      const { sut, surveyCollection } = await makeSut()
      const document = await surveyCollection.insertOne(makeFakeSurveyParamsWithDate())
